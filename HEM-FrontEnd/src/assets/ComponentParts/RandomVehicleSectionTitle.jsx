export default function RandomVehicleSectionTitle() {
    return (
      <div className="flex px-[10%] p-20 justify-evenly font-hemautoFontPlay">
        <h3 className="text-8xl text-center font-extrabold">
          MAKES & MODELS
        </h3>
        <div>
          <h2 className="text-3xl text-center text-hemautoLightGray pb-4">
            We provide Topeka, KS & the surrounding area with<br></br>
            high quality full-service automotive maintenance & repair<br></br>
            for most makes & models from the 21st century.
          </h2>
          <p className="text-center text-xl text-hemautoLightGray pb-2">
            For service information on vehicles manufactured 2000 & before
          </p>
          <div className="text-center text-hemautoGreen text-xl">
            <button>
              Contact US
            </button>
          </div>
        </div>
      </div>
    );
}
